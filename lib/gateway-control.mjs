...
/**
 * Restart the gateway.
 * Promote path (installDir provided): `openclaw gateway restart --wait` via the
...
export async function restartGateway(installDir, opts = {}) {
  if (installDir) {
    process.stdout.write('  → openclaw gateway restart --wait ...\n');
    runOpenclawGateway(installDir, ['restart', '--wait'], { timeout: 90000, ...opts });
    return;
  }
  if (detectPlatform() === 'macos') {
